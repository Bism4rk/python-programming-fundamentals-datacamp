...
'''
O código acima demonstra como criar um dicionário para armazenar dados, no caso pesos de pinguins fêmeas, onde cada chave é uma espécie e o valor é uma lista de tuplas contendo o sexo e a massa corporal. A verificação para ver se a espécie já existe no dicionário garante que não sobrescrevemos dados existentes, e a adição de tuplas permite armazenar múltiplas entradas para cada espécie.
'''